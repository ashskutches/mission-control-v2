...
import { RefreshCw, CheckCircle, XCircle, Clock, AlertTriangle, ChevronDown, ChevronUp, Loader } from "lucide-react";
            ...
            {(grouped[category] ?? []).map(item => {
              ...
              return (
                <div key={item.id} style={{
                  ...
                  border: item.status === "broken"    ? "1px solid rgba(244,63,94,0.25)"
                  ...
                  >
                    ...
                    {!item.credentials_ok && item.env_vars?.length > 0 && (
                      <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 6px", borderRadius: 4, background: "rgba(244,63,94,0.15)", color: "#f43f5e" }}>
                        NO CREDS
                      </span>
                    )}
                    <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 4, background: cfg.bg, color: cfg.color, letterSpacing: "0.06em" }}>
                      {cfg.label.toUpperCase()}
                    </span>
                    {isExpanded ? <ChevronUp size={13} style={{ color: "#64748b", flexShrink: 0 }} /> : <ChevronDown size={13} style={{ color: "#64748b", flexShrink: 0 }} />}
                  </div>

                  {/* Expanded detail */}
                  {isExpanded && (
                    <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
                      {item.description && <p style={{ fontSize: 12, color: "#94a3b8", margin: 0 }}>{item.description}</p>}
                      ...
                      {item.env_vars?.length > 0 && (
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {item.env_vars.map(v => (
                              <code key={v} style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "rgba(255,255,255,0.05)", color: "#94a3b8" }}>{v}</code>
                            ))}
                          </div>
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {item.agent_tools.map(t => (
                              <span key={t} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: "rgba(56,189,248,0.08)", color: "#38bdf8" }}>{t}</span>
                            ))}
                          </div>
                        </div>
                      )}

                      {item.notes && <p style={{ fontSize: 11, color: "#64748b", margin: 0, fontStyle: "italic" }}>{item.notes}</p>}
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap", alignItems: "center" }}>
                        ...
                        {item.docs_url && (
                          <a href={item.docs_url} target="_blank" rel="noopener noreferrer" style={{
