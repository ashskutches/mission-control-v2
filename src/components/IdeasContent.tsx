...
import {
  ...
  Workflow, BarChart2, ShoppingBag, Megaphone, Database, Rocket, Sparkles, Check, Wrench, Bug,
} from \"lucide-react\";
...
function IdeaModal({ initial, onClose, onSaved }: { initial?: Partial<Idea>; onClose: () => void; onSaved: () => void }) {
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97 }}
        ...
        <form onSubmit={handleSave} style={{ display: \"flex\", flexDirection: \"column\", gap: 14 }}>
          <div>
            ...
            <input required value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder=\"e.g. Build a weekly inventory report\" style={inputStyle} />
          </div>
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
              placeholder=\"What is this idea? Why is it valuable?\"
...
function IdeaCard({ idea, onEdit, onDelete, onStatusChange }: {
  idea: Idea; onEdit: () => void; onDelete: () => void; onStatusChange: (status: IdeaStatus) => void;
}) {
    ...
    >
      <div style={{ display: \"flex\", alignItems: \"flex-start\", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {idea.description && <p style={{ color: \"#777\", fontSize: 11, margin: 0, lineHeight: 1.5 }}>{idea.description}</p>}
        </div>
        {hovered && (
    ...
    <div>
      ...
      <div style={{ display: \"flex\", alignItems: \"center\", justifyContent: \"space-between\", marginBottom: 18, flexWrap: \"wrap\", gap: 10 }}>
        <div style={{ display: \"flex\", alignItems: \"center\", gap: 8 }}>
          ...
          {agentCount > 0 && (
            <span style={{ fontSize: 10, color: \"#64748b\" }}>{agentCount} from agents</span>
          )}
        </div>
        <button
          onClick={() => setModal({ open: true })}
          style={{ display: \"flex\", alignItems: \"center\", gap: 6, padding: \"7px 14px\", background: \"rgba(167,139,250,0.12)\", border: \"1px solid rgba(167,139,250,0.3)\", borderRadius: 8, color: \"#a78bfa\", cursor: \"pointer\", fontWeight: 800, fontSize: 12 }}>
      ...
      <div style={{ display: \"flex\", gap: 8, marginBottom: 18, flexWrap: \"wrap\" }}>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}
          style={{ background: \"rgba(255,255,255,0.05)\", border: \"1px solid rgba(255,255,255,0.1)\", borderRadius: 7, color: \"#fff\", fontSize: 11, padding: \"5px 10px\", cursor: \"pointer\", outline: \"none\" }}>
        ...
        <span style={{ marginLeft: \"auto\", fontSize: 11, color: \"#475569\", alignSelf: \"center\" }}>{filtered.length} idea{filtered.length !== 1 ? \"s\" : \"\"}</span>
      </div>

      {/* Grid */}
      {loading ? (
        <p style={{ color: \"#555\", textAlign: \"center\", fontSize: 13, paddingTop: 40 }}>Loading…</p>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: \"center\", paddingTop: 60, opacity: 0.4 }}>
          <Lightbulb size={36} color=\"#a78bfa\" style={{ marginBottom: 12 }} />
