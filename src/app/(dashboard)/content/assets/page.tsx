...
function FileCard({ file, selected, onToggle, onTagAdd, onTagRemove }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
      style={{ ...CARD, padding: "0.875rem", border: selected ? `1px solid ${ACCENT}40` : "1px solid rgba(255,255,255,0.07)", background: selected ? `${ACCENT}06` : "rgba(255,255,255,0.03)", transition: "all 0.12s" }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", marginBottom: "0.6rem" }}>
        <button onClick={onToggle} style={{ flexShrink: 0, width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${selected ? ACCENT : "rgba(255,255,255,0.15)"}`, background: selected ? ACCENT : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }} aria-label={selected ? "Deselect" : "Select"}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem", minHeight: 20 }}>
        {(file.tags ?? []).map(tag => <TagBadge key={tag} tag={tag} onRemove={() => onTagRemove(tag)} />)}
        {showTagInput ? (
...
export default function AssetTaggerPage() {
  ...
  const addTag = (fileId: string, tag: string) => setTagMap(prev => { const current = prev[fileId] ?? files.find(f => f.id === fileId)?.tags ?? []; if (current.includes(tag)) return prev; return { ...prev, [fileId]: [...current, tag] }; });
  const removeTag = (fileId: string, tag: string) => setTagMap(prev => { const current = prev[fileId] ?? files.find(f => f.id === fileId)?.tags ?? []; return { ...prev, [fileId]: current.filter(t => t !== tag) }; });
  const batchTag = (tag: string) => setTagMap(prev => { const next = { ...prev }; selectedIds.forEach(id => { const current = prev[id] ?? files.find(f => f.id === id)?.tags ?? []; if (!current.includes(tag)) next[id] = [...current, tag]; }); return next; });
    ...
    <div>
      ...
      {loading ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.75rem" }}>
          {[...Array(6)].map((_, i) => <div key={i} style={{ height: 120, background: "rgba(255,255,255,0.03)", borderRadius: 12, animation: "pulse 1.5s infinite" }} />)}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ ...CARD, textAlign: "center", padding: "3rem", opacity: 0.5 }}>
          <FolderOpen size={32} color="#475569" style={{ marginBottom: "0.75rem" }} />
          <p style={{ color: "#475569", fontSize: 13 }}>
            {files.length === 0 ? "No files loaded — make sure Google Drive is connected in Integrations." : "No files match your filters."}
          </p>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.75rem" }}>
          {filtered.map((file, i) => (
            <motion.div key={file.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}>
              <FileCard file={file} selected={selectedIds.has(file.id)} onToggle={() => toggleSelect(file.id)}
                onTagAdd={tag => addTag(file.id, tag)} onTagRemove={tag => removeTag(file.id, tag)} />
            </motion.div>
          ))}
        </div>
      )}
