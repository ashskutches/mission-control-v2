...
import { SearchCheck, RefreshCw, Check, X } from "lucide-react";
    ...
    >
      ...
      <div className="p-3">
        <div className="is-flex is-align-items-flex-start" style={{ gap: "0.5rem" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="is-flex is-align-items-center mb-1" style={{ gap: "0.4rem", flexWrap: "wrap" }}>
              <span className="tag is-rounded" style={{ fontSize: "9px", background: `${color}18`, color, fontWeight: 800 }}>
            ...
            <p className="has-text-white" style={{ fontWeight: 700, fontSize: "0.82rem", marginBottom: "0.2rem" }}>
              {insight.title}
            </p>
            {insight.body && (
              <p style={{ fontSize: "0.75rem", color: "#94a3b8", lineHeight: 1.5 }}>{insight.body.slice(0, 120)}{insight.body.length > 120 ? "…" : ""}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-2 pt-2 is-flex is-align-items-center is-flex-wrap-wrap" style={{ borderTop: "1px solid rgba(255,255,255,0.05)", gap: "0.4rem" }}>
          {insight.status === "new" && (
            <>
              <button onClick={() => act("accepted")} disabled={acting} className="button is-small"
              ...
              <button onClick={() => setRejecting(!rejecting)} disabled={acting} className="button is-small"
          ...
          {insight.status === "in_progress" && (
            <button onClick={() => act("completed")} disabled={acting} className="button is-small"
              style={{ background: "rgba(34,197,94,0.12)", color: "#22c55e", border: "1px solid rgba(34,197,94,0.25)", fontWeight: 700, fontSize: "11px", gap: "0.3rem" }}>
        ...
        <AnimatePresence>
          {rejecting && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-2">
              <input className="input is-small" placeholder="Why reject? (helps the agent learn)" value={rejectNote}
                onChange={e => setRejectNote(e.target.value)}
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#e2e8f0", fontSize: "11px" }}
                onKeyDown={e => { if (e.key === "Enter") act("rejected", rejectNote || undefined); }}
                autoFocus
              />
              <div className="is-flex mt-1" style={{ gap: "0.4rem" }}>
                <button onClick={() => act("rejected", rejectNote || undefined)} className="button is-small"
                  style={{ fontSize: "11px", color: "#f43f5e", background: "rgba(244,63,94,0.1)", border: "none" }}>Confirm</button>
...
function TypingDots() {
  return (
    <div style={{ display: "flex", gap: 4, alignItems: "center", padding: "8px 12px" }}>
      {[0, 1, 2].map(i => (
        <motion.span key={i}
          style={{ width: 5, height: 5, borderRadius: "50%", background: AGENT_COLOR, display: "block" }}
...
function EmbeddedChat({ agentId, agentName, agentEmoji = "🤖", metrics, insights }: {
  ...
  // Boot: find or create section conversation
  useEffect(() => {
    if (!agentId) return;
    (async () => {
      try {
        ...
        const existing = Array.isArray(convos)
          ? convos.find((c: any) => c.title?.includes("[seo-commerce]"))
          : null;
        let cid: string;
        if (existing) { cid = existing.id; setIsFirstMessage(false); }
          ...
          const cr = await fetch(`${BOT_URL}/admin/chat/conversations`, {
            ...
            body: JSON.stringify({ agent_id: agentId, title: `[seo-commerce] SEO Dashboard Chat` }),
          });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "rgba(0,0,0,0.25)", borderRadius: 14, border: `1px solid ${AGENT_COLOR}18`, minWidth: 0 }}>
      {/* Chat Header */}
      <div style={{ padding: "0.875rem 1.1rem", borderBottom: `1px solid ${AGENT_COLOR}15`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <div style={{ width: 30, height: 30, borderRadius: "50%", background: `${AGENT_COLOR}18`, border: `1px solid ${AGENT_COLOR}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0 }}>
          {agentEmoji}
        </div>
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ margin: 0, fontSize: "9px", color: "#475569" }}>SEO lead · context loaded</p>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "1rem", minWidth: 0 }} className="custom-scrollbar">
        {displayMessages.length === 0 && !sending ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", opacity: 0.35, gap: 8, textAlign: "center" }}>
            ...
            <p style={{ color: "#ccc", fontSize: "12px", margin: 0 }}>Ask {agentName} about the SEO data on this page.</p>
            <p style={{ color: "#475569", fontSize: "10px", margin: 0 }}>They know about every metric and insight shown here.</p>
          </div>
        ) : (
          <AnimatePresence initial={false}>
            {displayMessages.map(msg => {
              ...
              return (
                ...
                >
                  {!isUser && (
                    <div style={{ width: 24, height: 24, borderRadius: "50%", background: `${AGENT_COLOR}18`, border: `1px solid ${AGENT_COLOR}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0 }}>
                      {agentEmoji}
                    </div>
                  )}
                  <div style={{
                    ...
                    background: isUser ? `linear-gradient(135deg, ${AGENT_COLOR}28, ${AGENT_COLOR}18)` : "rgba(255,255,255,0.05)",
                    border: isUser ? `1px solid ${AGENT_COLOR}30` : "1px solid rgba(255,255,255,0.07)",
                    color: "#eee", fontSize: "0.8rem", lineHeight: 1.55, wordBreak: "break-word", overflowWrap: "anywhere",
        ...
        <AnimatePresence>
          {sending && (
            <motion.div key="typing" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <div style={{ width: 24, height: 24, borderRadius: "50%", background: `${AGENT_COLOR}18`, border: `1px solid ${AGENT_COLOR}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>
                {agentEmoji}
              </div>
              <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "13px 13px 13px 4px" }}>
                <TypingDots />
              </div>
            </motion.div>
          )}
      ...
      {/* Input */}
      <div style={{ padding: "0.75rem 1rem", borderTop: `1px solid ${AGENT_COLOR}15`, flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 7, alignItems: "flex-end", background: "rgba(255,255,255,0.04)", border: `1px solid ${AGENT_COLOR}20`, borderRadius: 10, padding: "6px 6px 6px 10px" }}>
          <textarea ref={textareaRef} value={input}
            onChange={e => { setInput(e.target.value); e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 100) + "px"; }}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
            placeholder={`Ask ${agentName} about SEO…`} disabled={sending} rows={1}
          ...
          <button onClick={handleSend} disabled={!input.trim() || sending} aria-label="Send"
            style={{ width: 28, height: 28, borderRadius: 7, border: "none", flexShrink: 0, transition: "all 0.18s", display: "flex", alignItems: "center", justifyContent: "center", cursor: input.trim() && !sending ? "pointer" : "default",
              background: input.trim() && !sending ? `linear-gradient(135deg, ${AGENT_COLOR}, ${AGENT_COLOR}cc)` : "rgba(255,255,255,0.06)",
    ...
    <div style={{ padding: "1.25rem 1.5rem", height: "calc(100vh - 60px)", display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
      ...
      <div className="is-flex is-justify-content-space-between is-align-items-center mb-4" style={{ flexShrink: 0 }}>
        <div className="is-flex is-align-items-center" style={{ gap: "0.65rem" }}>
          <SearchCheck size={20} color="#38bdf8" />
          <div>
            <h1 className="has-text-white" style={{ fontWeight: 800, fontSize: "1.25rem", lineHeight: 1 }}>SEO</h1>
            <p className="has-text-grey-light" style={{ fontSize: "0.75rem" }}>Commerce · Search · Traffic · Rankings</p>
          </div>
        </div>
        <button onClick={() => { fetchInsights(); fetchMetrics(); setRefreshTrigger(t => t + 1); }}
          className="button is-small is-ghost" style={{ color: "#64748b" }}>
          <RefreshCw size={13} />
        </button>
      </div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 380px", gap: "1.25rem", minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
          ...
          {/* Metrics */}
          <div style={{ flexShrink: 0, marginBottom: "1rem" }}>
            <SectionMetricsPanel sectionId="seo" agentName={assignedAgent?.name} refreshTrigger={refreshTrigger} />
          </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", minWidth: 0 }} className="custom-scrollbar">
            ...
            <div className="is-flex mb-3" style={{ gap: "0.35rem", borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: "0.5rem" }}>
              {STATUS_FILTERS.map(s => (
                <button key={s} onClick={() => setStatusFilter(s)} className="button is-small"
                  style={{
            ...
            <AnimatePresence mode="popLayout">
              {filtered.length === 0 ? (
                <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  style={{ fontSize: "0.82rem", color: "#334155", textAlign: "center", padding: "2rem 0" }}>
