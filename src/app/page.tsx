...
function GlobalDriveSettings() {
  const [status, setStatus]     = useState<{ email: string; created_at: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (params.get("drive_connected") === "__global__") {
      fetchStatus();
      window.history.replaceState({}, "", window.location.pathname);
    }
    if (params.get("drive_error")) {
      setError(decodeURIComponent(params.get("drive_error") ?? ""));
      window.history.replaceState({}, "", window.location.pathname);
    }
  }, [fetchStatus]);

  const handleConnect = () => {
    window.location.href = `${BOT_URL}/auth/drive?agent_id=__global__`;
  };

  const handleDisconnect = async () => {
    if (!confirm("Disconnect Google Drive? All agents will lose Drive access.")) return;
    setRemoving(true);
    try {
      await fetch(`${BOT_URL}/admin/settings/drive`, { method: "DELETE" });
      setStatus(null);
    } catch (e: any) {
    ...
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "1.25rem" }}>
      ...
      {loading ? (
        ...
        <div>
          ...
          <button onClick={handleDisconnect} disabled={removing} className="button is-small is-dark"
        ...
        <div>
          ...
          <button onClick={handleConnect} className="button is-small"
            style={{ background: "rgba(255,140,0,0.12)", border: "1px solid rgba(255,140,0,0.3)", color: "#ff8c00", fontWeight: 700, fontSize: 11 }}>
