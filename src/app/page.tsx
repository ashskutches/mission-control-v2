...
function parseStockItems(raw: string) {
  return raw.split(", ").map((item) => {
    const match = item.match(/^(.*?):\s*(-?\d+)\s*left$/);
    return { name: match ? match[1] : item, qty: match ? parseInt(match[2]) : 0 };
  });
...
export default function CommandCenter() {
  ...
  useEffect(() => {
    supabase.from("cost_log").select("cost_usd").then(({ data }: { data: any[] | null }) => {
      if (data) setTotalCost(data.reduce((s: number, r: any) => s + (r.cost_usd || 0), 0));
    });
  }, []);
  ...
  const icon = (a: string) => ({ discord_message: "⚡", telegram_message: "✈️", memory_save: "🧠", tool_use: "🔧" }[a] || "💬");

  const stockItems = shopify?.lowStock ? parseStockItems(shopify.lowStock) : [];
  const filtered = stockItems.filter((s: any) => stockFilter === "critical" ? s.qty < 0 : stockFilter === "out" ? s.qty === 0 : true);
  const criticalCount = stockItems.filter((s: any) => s.qty < 0).length;
  const outCount = stockItems.filter((s: any) => s.qty === 0).length;
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 32 }}>
        ...
        {!["command", "store"].includes(activeTab) && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh", flexDirection: "column", gap: 12 }}>
            <div style={{ fontSize: 40 }}>{NAV_ITEMS.find((n: any) => n.id === activeTab)?.icon}</div>
            <div style={{ fontSize: 20, fontWeight: 600, color: "#333" }}>{NAV_ITEMS.find((n: any) => n.id === activeTab)?.label}</div>
